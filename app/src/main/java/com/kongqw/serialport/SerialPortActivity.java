...
public class SerialPortActivity extends AppCompatActivity implements OnOpenSerialPortListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toggleButton = (ToggleButton)findViewById(R.id.et_send_hex);
        ...
        String baud_rate = getIntent().getStringExtra(BAUD_RATE);
        Log.i(TAG, "onCreate: device = " + device+"--baud_rate-"+baud_rate);
        if (null == device) {
            finish();
            return;
        }
        ...
        boolean openSerialPort = mSerialPortManager.setOnOpenSerialPortListener(this)
                .setOnSerialPortDataListener(new OnSerialPortDataListener() {
                    ...
                    public void onDataReceived(byte[] bytes) {
                        Log.i(TAG, "onDataReceived [ byte[] ]: " + CHexConver.byte2HexStr_blank(bytes));
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if(isHex){
                                    showToast(String.format("接收\n%s", CHexConver.byte2HexStr_blank(finalBytes)));
                                }else {
                    ...
                    public void onDataSent(byte[] bytes) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if(isHex){
                                    showToast(String.format("接收\n%s", CHexConver.byte2hex(finalBytes)));
                                }else {
                                    showToast(String.format("接收\n%s", new String(finalBytes)));
                                }
                            }
                ...
                .openSerialPort(device.getFile(), Integer.valueOf(baud_rate));
    ...
    public void onSend(View view) {
        ...
        byte[] sendContentBytes ;

        if(isHex){
            sendContentBytes =CHexConver.String2HexBytes(sendContent);
        }else {
            sendContentBytes = sendContent.getBytes();
        }
